...
- Exception: `packages/cloud/src/auth-runtime.ts` stores its managed authentication runtime on `globalThis` because Next.js compiles the custom server and route handler as separate module instances. `packages/cloud/src/server.ts` is the sole lifecycle owner and awaits disposal after draining requests; no other cloud state may use this escape hatch.
