...
Set `CLOUD_CLIENT_IP_HEADER` to one authoritative, single-value client address header written by the edge proxy; `fly-client-ip` is the Fly Proxy value. The origin must not be directly reachable by clients, and the proxy must replace rather than append or preserve a caller-supplied value. Better Auth uses this header for per-client rate limits and session metadata.
