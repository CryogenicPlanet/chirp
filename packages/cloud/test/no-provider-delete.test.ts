...
describe("provider deletion source tripwire", () => {
	test("keeps provider deletion confined to its explicit worker capability", async () => {
		...
		const forbidden =
			/\b(?:deleteApp|deleteMachine|deleteVolume|deleteSecret|releaseIp)\b|(?:request|make)\("DELETE"|HttpClientRequest\.delete\(/;
		const violations: string[] = [];
		for (const file of files) {
			if (file.endsWith("/fly-deletion-api.ts") || file.endsWith("/cloudflare-deletion-api.ts")) continue;
