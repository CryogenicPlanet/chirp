...
export function AuthButtons({ invitation }: AuthButtonsProps) {
	...
	const signOut = () => {
		...
		Effect.tryPromise(() => auth.signOut()).pipe(
			Effect.tap((result) =>
				result.error ? Effect.sync(() => setError(result.error.message ?? "Sign-out failed")) : Effect.void,
			),
	...
	if (!invitation && session.isPending)
		return (
			<p aria-live="polite" className="text-xs text-muted-foreground">
				Checking your session…
			</p>
		);
	if (!invitation && session.data)
		return (
			<div className="grid min-w-0 gap-2">
				<div className="mb-1.5 grid min-w-0 gap-0.5">
					<strong className="text-[13px] font-medium">{session.data.user.name}</strong>
					<span className="text-xs text-muted-foreground [overflow-wrap:anywhere]">{session.data.user.email}</span>
				</div>
				<button
